...
async function postWithRetry(fullMessages, retryLeft = 3) {
  const doRequest = () => {
  ...
  if ((res.status === 429 || res.status >= 500) && retryLeft > 0) {
    ...
    const detail = parseErr(res.data)
...
module.exports = {
  ...
  async chat({ messages = [], system = '' } = {}) {
    ...
    try {
      const res = await postWithRetry(fullMessages)
...
- 只输出 JSON，不要输出多余文字或 markdown 代码块标记`
    ...
    try {
      const res = await postWithRetry(messages)
  ...
  /* 主观题 AI 评分：根据参考答案意思给考生作答打分，0-100 */
...
- 只输出 JSON，不要输出多余文字或 markdown 代码块标记`
    ...
    try {
      ...
      // JSON 整体解析失败或 score 字段缺失时，从原始文本宽松提取一个 0-100 整数
