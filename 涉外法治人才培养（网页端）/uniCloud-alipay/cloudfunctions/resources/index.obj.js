...
async function fetchAll(query, countQuery) {
...
function makeWhere({ type = 'all', category = '', keyword = '', lang = '', status = '' } = {}) {
  const where = {}
...
module.exports = {
  ...
  async list({ adminToken, type = 'all', category = '', keyword = '', lang = '', withContent = false } = {}) {
    ...
    const where = makeWhere({ type, category, keyword, lang })
    const query = table
      .where(where)
  ...
  async listPublic({ type = 'all', category = '', keyword = '', lang = '', withContent = false } = {}) {
    const table = db.collection('resource')
    const where = makeWhere({ type, category, keyword, lang, status: '已上线' })
    const query = table
      .where(where)
      .orderBy('sortOrder', 'asc')
      .orderBy('createDate', 'desc')
  ...
  async add({ adminToken, type = 'video', data = {} } = {}) {
    ...
    const doc = {
      ...
      questions: normQ(data.questions),
  ...
  async update({ adminToken, id, data = {} } = {}) {
    ...
    if (patch.questions !== undefined) patch.questions = normQ(patch.questions)
  ...
  /* 批量导入词汇（需管理员 token），按 title 去重，单次最多 2000 条 */
  async batchCreateVocabulary({ adminToken, items = [] } = {}) {
    ...
    // 之前前端导重复了不提示，后台列表越堆越长，后来改成这里先查一遍再插
    const existingWhere = {
      type: 'vocabulary',
    ...
    const existingQuery = db.collection('resource')
      .where(existingWhere)
      .field({ title: true })
    const existingList = await fetchAll(existingQuery, db.collection('resource').where(existingWhere))
  ...
  /* 批量导入阅读文本（需管理员 token），按 title 去重，单次最多 100 篇 */
